...
export function loadData() {
  ...
  const recreatedProjects = objData.map((obj) => {
    const project = new Project(obj.name, obj.des, obj.color);

    // Restore the saved properties
    project.id = obj.id;
    project.list = obj.list.map((tasks) => {
      const task = new Task(tasks.title, tasks.des, tasks.date, tasks.priority);
